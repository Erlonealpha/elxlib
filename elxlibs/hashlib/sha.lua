...
do
	local function mul(src1, src2, factor, result_length)
		...
		local result, carry, value, weight = table_create(result_length), 0, 0, 1
	...
	repeat
		...
		repeat
			...
			if d * d > p then
				...
				R = mul(table_create(1, math.floor(R)), one, 1, 2)
...
local function sha256ext(width, message)
	...
	local function partial(message_part)
		if message_part then
			...
			if tail then
				local final_blocks = table_create(10) --{tail, "\128", string.rep("\0", (-9 - length) % 64 + 1)}
				final_blocks[1] = tail
				final_blocks[2] = "\128"
...
local function sha512ext(width, message)
	...
	local function partial(message_part)
		if message_part then
			...
			if tail then
				local final_blocks = table_create(3) --{tail, "\128", string.rep("\0", (-17-length) % 128 + 9)}
				final_blocks[1] = tail
				final_blocks[2] = "\128"
...
local function md5(message)
	...
	local function partial(message_part)
		if message_part then
			...
			if tail then
				local final_blocks = table_create(3) --{tail, "\128", string.rep("\0", (-9 - length) % 64)}
				final_blocks[1] = tail
				final_blocks[2] = "\128"
...
local function keccak(block_size_in_bytes, digest_size_in_bytes, is_SHAKE, message)
	...
	-- Create an instance (private objects for current calculation)
	local tail, lanes_lo, lanes_hi = nil, table_create(25, 0), hi_factor_keccak == 0 and table_create(25, 0)
	tail = ""
	local result
...
local function bin2base64(binary_string)
	...
	local result = table_create(math.ceil(stringLength / 3))
	local length = 0
